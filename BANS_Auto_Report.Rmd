---
title: "Bank Swallow Monitoring Report for the Fort Funston Population at Golden Gate National Recreation Area (DRAFT)"
...
maxBurSummary <- BT_Surveys %>% 
  ...
  summarise(
    ...
    Total_of_Avgs = max(TotBur, na.rm = TRUE))    # Max for TotBur column
...
maxBurSummary_long <- maxBurSummary %>% 
  pivot_longer(cols = starts_with("Av"),
               names_to = "Area",
               values_to = "Max_Count") %>% 
  separate(Area, into = c("Avg", "Area"), sep = "_") %>%  #fixes names from Ave_AreaA to AreaA
  select(Year, Area, Max_Count)

#Trends Analysis
MaxBur_lm <- lm(Total_of_Avgs ~ Year, maxBurSummary)
...
# Abstract
The Fort Funston breeding colony of bank swallows has been observed since the early 1900’s. Habitat loss and other factors led to bank swallows being listed as a threatened species under the California Endangered Species Act in 1989. The Fort Funston breeding colony of bank swallows may be one of only two remaining coastal breeding sites in California. Since 2020, the bank swallow colony has shifted locations along 's bluffs over the years, with bank swallows mainly nesting at Phillip Burton Memorial Beach which is just south of the Golden Gate National Recreation Area boundary. The total number of bank swallow burrows at the Fort Funston colony has been declining since 2000. **!!Following sentence may not be applicable every year!!** In `r TargetYr`, there were a max of `r HighestMaxBurs_Count` bank swallow burrows counted at `r HighestMaxBurs_Area`,  which is consistent, though slightly lower than burrow numbers in that area since 2019. A minimum of `r AreaMostActBur_Quantity` burrows were determined to be active in `r AreaMostActBur`, and one burrow was active in Area B above the rock revetment at the north end of Fort Funston. In addition, we enhanced burrowing habitat by drilling burrows for the second year in a row, removing ice plant from bluff faces, and placed temporary signage to keep people from disturbing active nesting areas.
...
![**Figure 1:** Map of Fort Funston bank swallow monitoring locations. Text between Monitoring Boundary Sectors indicates the monitoring area name.](Images/FOFU_Map.png)
...
BurrowsByYear <- lm(Total_of_Avgs~Year,maxBurSummary)

#Min Burrows
AvgBur_Plot <- ggplot(data = minBurSummary, aes(x = Year, y = Min_Burrows)) +
  geom_line() +
  geom_point() +
  geom_smooth(method = lm) +
...
#Avg Burrows
AvgBur_Plot <- ggplot(data = avgBurSummary, aes(x = Year, y = Avg_Burrows)) +
  geom_line() +
  geom_point() +
  geom_smooth(method = lm) +
...
Burrow_Dist <- ggplot(maxBurSummary_long, aes(x = Year, y = Max_Count, fill = Area)) +
  geom_col() +
  labs(title = "Distribution of Burrows across Areas by Year",
...
BurCountByMonth_Plot <- ggplot(monthlyAvg, aes(x = Month, y = MonthAvg_Bur))+
  geom_col()+
...
currentYrBySector_Plot <- ggplot(AvgBurByArea_long, aes(x = Area, y = AvgBurs)) +
  geom_col() +
  labs(title = paste(TargetYr," Average Burrows by Sector"), x = "Area", y = "Annual Average Burrows")

#Burrow counts over the Target Year
currentYrByMonth_Plot <- ggplot(currentYr_BT, aes(x = Date, y = TotBur)) +
  geom_line() +
  geom_point() +
  scale_y_continuous(limits = c(0, maxBur + 10)) +
  labs(title = paste(TargetYr, "Burrows Count by Month "), x = "Month", y = "Burrows")
```
# Results
## Burrow Transects
We conducted `r numBT_Surveys` burrow count surveys from `r startBT` to `r endBT`. The `r HighestMaxBurs_Area` was the area with the highest max burrow count of `r HighestMaxBurs_Count`. 

## Burrow Activity Surveys
We conducted `r numBA_Surveys` burrow activity surveys from `r startBA` to `r endBA`.

![**Figure 2:** Spatial distribution of max burrow counts across sampling areas by year.](Plots/All Years/Burrows_Distribution.png)


![**Figure 3:** Maximum burrow counts across all years monitored. ](Plots/All Years/MaxBurrowCountsbyYear.png)
...
# Potential Plots of Interest

![](Plots/Current Year/Burrows by Month.png)
